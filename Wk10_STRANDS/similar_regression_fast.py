# -*- coding: utf-8 -*-
"""
Created on Sat Sep 11 22:48:49 2021
...
def similar_regression_fast(edge, df_train, clf, 
                           ...
                           cutoff = 1):
    """
    Get most similar edge from training map with at least cutoff datapionts
    ...
    X = np.array(df_predict[["edge_length_diff", "origin_connections_diff", 
                            "target_connections_diff", "total_connections_diff", 
                            "max_angle_diff", "sum_angle_diff"]])
    df_predict["ks"] = clf.predict(X)
    
    #4) Sort array so that top values are most similar (lowest class0_prob)
    df_predict = df_predict.sort_values("ks", ascending = True).reset_index(drop=True)
    index = 0
