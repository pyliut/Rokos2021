# -*- coding: utf-8 -*-
"""
Created on Sat Sep 11 10:28:48 2021
...
def ks_dataloader(df, filename = "aaf_map.yaml",metric = "difference",
                  threshold = 0.3, threshold_metric = "ks",
                  cutoff = 1, verbose = False, report_interval = 5000):
    """
    ...
    for i in range(n_edges-1):
        ...
        for j in range(i+1,n_edges):
            ...
            #5b) KS values
            ks_raw,pval_raw = sp.stats.ks_2samp(t_op1,t_op2)
            df_ks_diff["ks"][index] = ks_raw
            df_ks_diff["pval"][index] = pval_raw
            if threshold_metric == "ks":
                df_ks_diff["same_cluster"][index] = ks_raw <= threshold
            elif threshold_metric == "pval":
                df_ks_diff["same_cluster"][index] = pval_raw >= threshold
            else:
                raise ValueError("Invalid threshold_metric")
            #5c) Spatial features
