# -*- coding: utf-8 -*-
"""
Created on Tue Aug 31 15:31:42 2021
...
def Normal_Gamma_bayes_initial(t_obs, mu_0 = 10, beta = 5, a = 1, b = 2, 
                       n_initial=1, threshold = 0.9, plot_graph = True):
    """
    ...
    mean_start= 0.01
    mean_stop = ( np.abs( np.max(t_obs) )//5 )*5 + 5   #round up to nearest 5 secs
    mean_step = 0.01
    mean_test = np.arange(mean_start,mean_stop,mean_step)
    
    #1b) define a range of variances to test
    var_start= 0.01
    var_stop = ( np.abs( np.max(t_obs) )//5 )*5 + 5    #round up to nearest 5 secs
    var_step = 0.01
