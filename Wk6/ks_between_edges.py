...
def ks_between_edges(df, threshold = 0.05, min_samples = 20, similar = True):
    """
    Returns a pandas Dataframe that contains 2-sample KS statistic and p-values between each pair of edges
    threshold, min_samples, similar can be used to choose which entries to retreive

    Parameters
    ----------
    df : pandas Dataframe
        observations. columns = ["origin","target","edge_id","operation_time"]
    threshold : FLOAT, scalar
        Value between 0 & 1. Sets threshold for p-value. 
        The default is 0.05.
    min_samples : INT, scalar
        Positive integer. If an edge contains less than min_samples amount of samples, it will not be included in the output. 
        The default is 20.
    similar : BOOL
        If True, return entries with p_values greater than threshold.
        If False, return entries with p_values less than threshold. 
        The default is True.

    Returns
    -------
    similar_sorted : pandas DataFrame
        KS statistic & p-value between pairs of edges
