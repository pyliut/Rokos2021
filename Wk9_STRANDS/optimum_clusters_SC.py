# -*- coding: utf-8 -*-
"""
Created on Fri Aug 27 13:57:36 2021
...
def optimum_clusters_SC(distance_matrix, max_clusters = 15, method = "ss"):
    """
    ...
    s_list = []             #score list
    for i in n_clusters:
        clustering = SpectralClustering(affinity='precomputed', n_clusters = i)
    
        labels = clustering.fit_predict( 1-np.array(distance_matrix) )
        if method == "ss":
    ...
    if method == "ss":
        ...
        print("Optimum n_clusters:",n_clusters[np.argmax(s_list)])
        ...
        plt.ylabel("DB Index")
        print("Optimum n_clusters:",n_clusters[np.argmin(s_list)])
