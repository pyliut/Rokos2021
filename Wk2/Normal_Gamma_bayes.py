...
def Normal_Gamma_bayes(t_obs, mu_0 = 10, beta = 5, a = 1, b = 2, threshold = 0.9):
    ...
    mean_start= 0.1
    mean_stop = np.max( [ 2*(np.max(t_obs)//5)*5 + 5, 40, 3*(mu_0//5)*5] )   #round up to nearest 5 secs
    mean_step = 0.1
    mean_test = np.arange(mean_start,mean_stop,mean_step)
    
    #1b) define a range of variances to test
    var_start= 0.1
    var_stop = np.max( [ 2*(np.max(t_obs)//5)*5 + 5, 40, 3*(mu_0//5)*5] )   #round up to nearest 5 secs
    var_step = 0.1
